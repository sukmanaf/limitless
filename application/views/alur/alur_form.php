...
<div class="panel panel-flat">
    ...
    <div class="panel-body">
        ...
        <div class="form-group">
                    ...
                    <div class="col-lg-10">
                       <select name="dari" class="select-search">
                       <?php foreach ($get_status as $key => $value): ?>
                            <?php $sel = $value->id == $dari ? 'selected': ''; ?>
                            <option <?= $sel; ?> value="<?= $value->id  ?>"><?= $value->status ?></option>   
                       <?php endforeach ?>
                        </select>
                    </div>
                </div>
        ...
        <div class="form-group">
                    <label class="control-label col-lg-2">ke</label>
                    <div class="col-lg-10">
                       <select name="ke" class="select-search">
                       <?php foreach ($get_status as $key => $value): ?>
                            <?php $sel = $value->id == $ke ? 'selected': ''; ?>
        ...
        <a href="<?php echo site_url('alur') ?>" class="btn btn-danger"><i class="fas fa-arrow-circle-left"></i> Kembali</a>
        </div>
...
function uploadImage(image) {
        ...
        $.ajax({
            url:'<?php echo site_url('alur/upload_summernote');?>', //URL submit
   ...
   $("#postForm").submit(function(event){
        ...
        $.ajax({
            ...
            setTimeout(function () {
                    window.location.replace('<?php echo site_url('alur') ?>')
                       
                    }, 2000);
                //    $.ajax({
                //              url:'<?php echo site_url('alur/do_upload');?>', //URL submit
                //              type:"post", //method Submit
